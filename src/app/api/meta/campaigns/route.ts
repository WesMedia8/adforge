// ============================================
// AdForge — Meta Campaigns CRUD Route
// ============================================
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : 'Failed to fetch campaigns' },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Update status (pause/resume/delete)
    ...
    if (campaignData.daily_budget) {
      payload.daily_budget = String(Math.round(Number(campaignData.daily_budget) * 100));
    }
    if (campaignData.lifetime_budget) {
      payload.lifetime_budget = String(Math.round(Number(campaignData.lifetime_budget) * 100));
    }
    ...
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : 'Campaign operation failed' },
      { status: 500 }
    );
  }
}

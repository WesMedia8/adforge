...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create creative + ad in one step
      ...
      const creative = await createAdCreative(creds, {
        ...
        object_story_spec: {
          ...
          link_data: {
            ...
            call_to_action: adData.cta_type ? {
              type: adData.cta_type,
              value: { link: adData.link_url || adData.website_url },
            } : undefined,
          },
        },
      });
    ...
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : 'Ad operation failed' },
      { status: 500 }
    );
  }
}

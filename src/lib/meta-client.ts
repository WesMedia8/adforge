...
// ─── Credential type passed from API routes ───
...
// ─── Helper: extract creds from request headers ─
...
export async function getCampaign(creds: MetaCredentials, campaignId: string): Promise<MetaCampaign> {
  return metaFetch<MetaCampaign>(creds, `/${campaignId}`, {
...
export async function updateCampaignStatus(
  ...
  return metaFetch<{ success: boolean }>(creds, `/${campaignId}`, {
    method: 'POST',
    body: { status },
  });
}
...
export async function updateAdSetStatus(
  ...
  return metaFetch<{ success: boolean }>(creds, `/${adSetId}`, {
    method: 'POST',
    body: { status },
  });
}
...
export async function updateAdStatus(
  ...
  return metaFetch<{ success: boolean }>(creds, `/${adId}`, {
    method: 'POST',
    body: { status },
  });
}
